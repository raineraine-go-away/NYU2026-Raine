...
namespace UnityEditor.ShaderGraph.Drawing
{
        ...
        public void GenerateNodeEntries()
        {
            ...
            foreach (var type in NodeClassCache.knownNodeTypes)
            {
                ...
                if (titleAttribute != null)
                {
                    ...
                    if (node is LoopNode)
                    {
                        if (!(node as LoopNode).ExposeToSearcher())
                            continue;
                    }
            ...
            // Add registed local variables to the searcher window and enable profiler sampling if there is any registed variables.
            if (m_Graph.localVariables.Count > 0)
            {
                Profiler.BeginSample("SearchWindowProvider.GenerateNodeEntries.IterateRegistedLocalVariables");
                foreach (var localVar in m_Graph.localVariables)
                {
                    var node = Activator.CreateInstance<GetLocalVariableNode>();
                    AddEntries(node, new string[]{ "Registed Variables", "Get: " + localVar}, nodeEntries);
                }
                Profiler.EndSample();
            }
    ...
    class SearcherProvider : SearchWindowProvider
    {
        ...
        public bool OnSearcherSelectEntry(SearcherItem entry, Vector2 screenMousePosition)
        {
            ...
            m_Graph.owner.RegisterCompleteObjectUndo("Add " + ((node is LoopNode) ? "Loop Node": node.name));
            ...
            // Don't set varIndex or variableName if choosing the default GetLocalVariableNode
            if (node is GetLocalVariableNode getNode && entry.Name.Split(':')[0].Equals("Get"))
            {
                var localVar = entry.Name.Split(':')[1].TrimStart();
                getNode.varIndex = m_Graph.localVariables.IndexOf(localVar);
                getNode.variableName = localVar;
            }
            ...
            {
                (node as LoopStart).CreateGroup(ref m_Graph);
                (node as LoopStart).CreateEndNode(ref m_Graph);
            }
