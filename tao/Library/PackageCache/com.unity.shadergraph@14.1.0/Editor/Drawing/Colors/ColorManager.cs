...
namespace UnityEditor.ShaderGraph.Drawing.Colors
{
    ...
    class ColorManager
    {
        ...
        {
            NoColors noColorsProvider = new NoColors();
            noColorsProvider.ApplyColor(nodeView);
            activeProvider.ApplyColor(nodeView);
            
            if (nodeView.node is LocalVariableRegisterNode)
                nodeView.gvNode.titleContainer.style.backgroundColor = new UnityEngine.Color(0.141f, 0.125f, 0.250f);
            if (nodeView.node is GetLocalVariableNode)
                nodeView.gvNode.titleContainer.style.backgroundColor = new UnityEngine.Color(0.239f, 0.153f, 0.220f);
