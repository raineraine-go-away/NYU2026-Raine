...
namespace UnityEditor.ShaderGraph.Drawing
{
    sealed class MaterialGraphView : GraphView, IInspectable, ISelectionProvider
    {
        ...
        public override List<Port> GetCompatiblePorts(Port startAnchor, NodeAdapter nodeAdapter)
        {
            ...
            var initial = new HashSet<AbstractMaterialNode> { startSlot.owner };
            ...
            foreach (var candidateAnchor in ports.ToList())
            {
                ...
                // Search connected nodes to avoid register/get loop;
                if (startSlot.isOutputSlot)
                {
                    results.Clear();
                    PreviewManager.PropagateNodes(initial, PreviewManager.PropagationDirection.Upstream, results);
                    if (results.Any(n => n.GetType() == typeof(GetLocalVariableNode)))
                    {
                        if (results.Contains(candidateSlot.owner))
                        {
                            results.Clear();
                            continue;
                        }
                        if (candidateSlot.owner is LocalVariableRegisterNode)
                        {
                            if (results.Any(n => n.GetType() == typeof(GetLocalVariableNode) && (n as GetLocalVariableNode).variableName.Equals((candidateSlot.owner as LocalVariableRegisterNode).variableName)))
                            {
                                results.Clear();
                                continue;
                            }
                ...
                else
                {
                    ...
                    if (results.Any(n => n.GetType() == typeof(LocalVariableRegisterNode)))
                    {
                        if (results.Contains(candidateSlot.owner))
                        {
                            results.Clear();
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            if (evt.target is Edge)
            {
                ...
                bool canAddPortal = (int)target.output.GetSlot().concreteValueType > 8          // For now, portal nodes only support vector and boolean values
                                    && (int)target.output.GetSlot().concreteValueType < 14
                                    && !(target.input.GetSlot().owner is LocalVariableRegisterNode || target.output.GetSlot().owner is GetLocalVariableNode);       // Can't do this action when edge connected any portal nodes
                var portalNodeStatus = canAddPortal ? DropdownMenuAction.Status.Normal : DropdownMenuAction.Status.Disabled;
                evt.menu.AppendSeparator();
                evt.menu.AppendAction("Add Redirect Node", e => CreateRedirectNode(pos, target));
                evt.menu.AppendAction("Add Portal Nodes", e => CreatePortalNodes(target), portalNodeStatus);
            }
        }
        ...
        string portalPairName;
        List<IEdge> tmpEdges = new List<IEdge>();
        void CreatePortalNodes(Edge edgeTarget)
        {
            if (!ValidAddPortals(edgeTarget.output.GetSlot().owner))
            {
                Debug.LogError("This will cause the circular loop, please remove the empty get node upstream and continue");
                return;
            }
            ...
            tmpEdges.Clear();
            if (outputSlot.owner is RedirectNodeData redirectNode)
                outputSlot = NodeUtils.DepthFirstCollectRedirectNodeFromNode(redirectNode).slot;    // If the edge connect from a redirect node, search for the original node which is not a redirect node.
            graph.GetEdges(outputSlot.slotReference, tmpEdges);
            var position = edgeTarget.output.node.GetPosition();

            // Reference the existing register when add portal nodes or create new register.
            if (tmpEdges.Any(e => e.inputSlot.node is LocalVariableRegisterNode))
                portalPairName = ((LocalVariableRegisterNode)tmpEdges.Select(e => e.inputSlot.node).Where(n => n is LocalVariableRegisterNode).ElementAt(0)).variableName;
            else
                LocalVariableRegisterNode.Create(graph, position, outputSlot.slotReference, group, out portalPairName);

            GetLocalVariableNode.Create(graph, position, inputSlot.slotReference, group, portalPairName);
        }

        bool ValidAddPortals(AbstractMaterialNode sourceNode)
        {
            var source = new HashSet<AbstractMaterialNode> { sourceNode };
            var results = new HashSet<AbstractMaterialNode>();
            bool valid = true;
            results.Clear();
            PreviewManager.PropagateNodes(source, PreviewManager.PropagationDirection.Upstream, results);
            foreach (var node in results)
            {
                if (node is GetLocalVariableNode && graph.localVariables.Count() == 0 && ((GetLocalVariableNode)node).varIndex == 0)
                {
                    valid = false;
                }
            }
        ...
        public void RemoveFromGroupNode()
        {
            ...
            foreach (var element in selection.OfType<GraphElement>())
            {
                if (element.userData is IGroupItem && !(element.userData is LoopNode))
                {
        ...
        string SerializeGraphElementsImplementation(IEnumerable<GraphElement> elements)
        {
            ...
            {
                if (node is LoopNode)
                    CopyLoopNode(ref nodes, ref groups, node);
            }
        ...
        void CopyLoopNode(ref IEnumerable<AbstractMaterialNode> nodes, ref IEnumerable<GroupData> groups, AbstractMaterialNode node)
        {
            int loopIndex = (node as LoopNode).loopIndex;
            if (node is LoopStart)
            {
                int loopEndCount = graph.loopEndNodes.SelectValue().Count();
                if (loopEndCount >= loopIndex && loopIndex > 0)
                {
                    var endNode = graph.loopEndNodes.SelectValue().ElementAt(loopIndex - 1);
                    CopyLoopNode(ref nodes, endNode, loopIndex);
                }
            }
            else if (node is LoopEnd)
            {
                int loopStartCount = graph.loopStartNodes.SelectValue().Count();
                if (loopStartCount >= loopIndex && loopIndex > 0)
                {
                    var startNode = graph.loopStartNodes.SelectValue().ElementAt((node as LoopEnd).loopIndex - 1);
                    CopyLoopNode(ref nodes, startNode, (node as LoopEnd).loopIndex);
                }
            }
            CopyLoopGroup(ref groups, node.group);
        }
        void CopyLoopNode(ref IEnumerable<AbstractMaterialNode> nodes,  LoopNode nodeToAdd, int loopIndex)
        {
            if (!nodes.Contains(nodeToAdd) && (nodeToAdd.loopIndex == loopIndex))
            {
                var tempNodes = nodes.ToList();
                tempNodes.Add(nodeToAdd);
                nodes = tempNodes;
            }
        }
        void CopyLoopGroup(ref IEnumerable<GroupData> groups, GroupData groupData)
        {
            if (!groups.Contains(groupData))
            {
                var tempGroups = groups.ToList();
                tempGroups.Add(groupData);
                groups = tempGroups;
            }
        }
