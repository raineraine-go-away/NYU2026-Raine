        ...
        static void ForeachConnectedNode(AbstractMaterialNode node, PropagationDirection dir, Action<AbstractMaterialNode> action)
        {
            ...
            using (var tempSlots = PooledList<MaterialSlot>.Get())
            {
                ...
                else
                {
                    ...
                    {
                        if (node.owner.localVariables.Any() && node.owner.localVariables.Count > (node as GetLocalVariableNode).varIndex)
                        {
                            var registerNode = node.owner.GetLocalVariableRegisterNode(node.owner.localVariables[(node as GetLocalVariableNode).varIndex]);
                            registerNode?.GetInputSlots(tempSlots);
                            node.owner.ClearErrorsForNode(node);
                        }
                        else
                        {
                            node.owner.AddValidationError(node.objectId, "Variable register was deleted, this node will return 0.", Rendering.ShaderCompilerMessageSeverity.Warning);
                        }
                    }
                    else
                    {
                ...
                {
                    node.owner.getVarNodes.ForEach(getVarNode =>
                    {
                        if ((node as LocalVariableRegisterNode).variableName.Equals(getVarNode.variableName))
                            action(getVarNode);
                    });
                }
            ...
            using (var perMaterialPreviewProperties = PooledList<PreviewProperty>.Get())
            {
                ...
                foreach (var preview in m_PreviewsToDraw)
                {
                    ...
                    { // skip if the node doesn't have a preview expanded (unless it's master)
                        ...
                        if ((node != kMasterProxyNode) && (!node.hasPreview || !node.previewExpanded || node.nodeLOD == 2))
                            continue;
                    }
        ...
        void KickOffShaderCompilations()
        {
            ...
            using (var previewsToCompile = PooledHashSet<PreviewRenderData>.Get())
            {
                ...
                for (int i = 0; i < m_PreviewsNeedsRecompile.Count(); i++)
                {
                    ...
                    if (node.hasPreview && (node.previewExpanded && node.nodeLOD < 2) && !m_PreviewsCompiling.Contains(preview))
                    {
