        ...
        void Update()
        {
            ...
            try
            {
                ...
                if (graphObject.isDirty || wasUndoRedoPerformed)
                {
                    ...
                    // Any changes may influence the message on BlockNode of Vertex.
                    graphEditorView?.graphView?.graph?.ValidateVertexGDRP();
                }

                // Called again to handle changes from deserialization in case an undo/redo was performed
                graphEditorView.HandleGraphChanges(wasUndoRedoPerformed, true);
                graphObject.graph.ClearChanges();
