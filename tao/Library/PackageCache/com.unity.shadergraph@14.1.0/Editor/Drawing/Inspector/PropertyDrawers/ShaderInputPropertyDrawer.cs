    ...
    class ShaderInputPropertyDrawer : IPropertyDrawer
    {
        ...
        void BuildPropertyFields(PropertySheet propertySheet)
        {
            if (shaderInput is AbstractShaderProperty property)
            {
                ...
                isGlobalDeclaration = property.GetDefaultHLSLDeclaration() == HLSLDeclaration.Global;
        ...
        void HandleVector1ShaderProperty(PropertySheet propertySheet, Vector1ShaderProperty vector1ShaderProperty)
        {
            ...
            switch (vector1ShaderProperty.floatType)
            {
                case FloatType.Slider:
                    ...
                    propertySheet.Add(floatPropertyDrawer.CreateGUI(
                        ...
                        isGlobalDeclaration ? "Preview Value" : "Default Value",
            ...
            if (!isSubGraph && !isGlobalDeclaration)
            {
                var enumPropertyDrawer = new EnumPropertyDrawer();
                propertySheet.Add(enumPropertyDrawer.CreateGUI(
                    ...
                    (FloatPropertyType)vector1ShaderProperty.floatType,
                    "Mode",
                    FloatPropertyType.Default,
                    out var modePropertyEnumField));;
            }
        }
        enum FloatPropertyType { Default = FloatType.Default, Slider = FloatType.Slider, Integer = FloatType.Integer }
        ...
        void BuildKeywordFields(PropertySheet propertySheet, ShaderInput shaderInput)
        {
            ...
            {
                var togglePropertyDrawer = new ToggleDataPropertyDrawer();
                bool toggleEnabled = keyword.keywordDefinition != KeywordDefinition.Predefined;
                bool toggleValue = keyword.keywordScope == KeywordScope.Global || !toggleEnabled;
                propertySheet.Add(togglePropertyDrawer.CreateGUI(
                    newValue =>
                    {
                        this._preChangeValueCallback("Change Keyword enable state");
                        keyword.keywordScope = newValue.isOn ? KeywordScope.Global : KeywordScope.Local;
                    },
                    new ToggleData(toggleValue),
                    "Is Overridable",
                    out keywordScopeField));
                keywordScopeField.SetEnabled(toggleEnabled);
            }
