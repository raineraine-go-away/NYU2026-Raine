...
namespace UnityEditor.ShaderGraph
{
    ...
    internal struct SpaceTransform
    {
        ...
        public SpaceTransform(CoordinateSpace from, CoordinateSpace to, ConversionType type, bool normalize = false, int version = kLatestVersion, ShaderStageCapability stageCapability = ShaderStageCapability.None)
        {
            this.from = from;
            this.to = to;
            this.type = type;
    ...
    static class SpaceTransformUtil
    {
        ...
        {
            if (xform.stageCapability == ShaderStageCapability.Fragment)
            {
                sb.AddLine("#ifdef UNITY_GPU_DRIVEN_PIPELINE");
                switch (xform.type)
                {
                    case ConversionType.Position:
                        sb.AddLine(outputVariable, "float4x4 local2World = MakeInstanceMatrix(instance);");
                        sb.AddLine(outputVariable, "float4x4 world2Local = Inverse(local2World);");
                        sb.AddLine(outputVariable, " = TransformWorldToObject(", inputValue, ", world2Local);");
                        break;
                    case ConversionType.Direction:
                        if (xform.version <= 1)
                            xform.normalize = true;
                        sb.AddLine(outputVariable, " = TransformWorldToObjectDir(", inputValue, ", worldToObjectMatrix", xform.NormalizeString(), ");");
                        break;
                    case ConversionType.Normal:
                        sb.AddLine(outputVariable, "float4x4 local2World = MakeInstanceMatrix(instance);");
                        sb.AddLine(outputVariable, "float4x4 world2Local = Inverse(local2World);");
                        sb.AddLine(outputVariable, " = TransformWorldToObjectNormal(", inputValue, ", local2World, world2Local, ", xform.NormalizeString(), ");");
                        break;
                }
                sb.AddLine("#else");
            }
