        ...
        , IMayRequireOutputVGTiling
    {
        ...
        public void GenerateNodeCode(ShaderStringBuilder sb, GenerationMode generationMode)
        {
            ...
            foreach (var outSlot in asset.outputs)
            {
                ...
                // We should declare output variables for VGTiling.
                var vgTilingVariable = GetVGTilingVariableName(outSlot.id);
                if (vgTilingVariable != null)
                    sb.AppendLine("{0}2 {1};", asset.outputGraphPrecision.ToGenericString(), vgTilingVariable);
            }
            ...
            foreach (var outSlot in asset.outputs)
            {
                ...
                // We should add out VGTiling variable.
                var vgTilingVariable = GetVGTilingVariableName(outSlot.id);
                if (vgTilingVariable != null)
                    arguments.Add(vgTilingVariable);
            }
            ...
            if (m_StageCapability == ShaderStageCapability.Fragment)
                sb.Append(" UNITY_GDRP_INSTANCE_ARGUMENT");
            else
                sb.Append(" UNITY_GDRP_INSTANCE_ZERO_ARGUMENT");
            sb.Append(", materialOffset");
            sb.Append(");");
            sb.AppendNewLine();
        }
        ...
        public override void ValidateNode()
        {
            ...
            owner.messageManager?.AddOrAppendError(owner, objectId, new ShaderMessage("SubGraph", (ShaderCompilerMessageSeverity)2));
        }
        ...
        {
            return asset.GetOutputSlotVGTiling(slotId);
        }

        public string GetVGTilingVariableName(int slotId)
        {
            if (asset.GetOutputSlotVGTiling(slotId) == VGTiling.UV_INVALID)
                return null;
            return string.Format("_{0}_VGTiling_{1}", GetVariableNameForNode(), slotId);
        }
