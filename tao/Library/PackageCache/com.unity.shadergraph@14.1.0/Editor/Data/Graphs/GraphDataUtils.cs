            ...
            public static void ApplyActionLeafFirst(GraphData graph, Action<AbstractMaterialNode> action)
            {
                ...
                while (stack.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        stack.Push(node);
                        if (node.GetType() != typeof(GetLocalVariableNode))
                        {
                        ...
                        {
                            if (graph.localVariables.Count > (node as GetLocalVariableNode).varIndex && !(node as GetLocalVariableNode).variableName.Equals(""))
                                stack.Push(graph.GetNodeFromId(graph.registerIds[(node as GetLocalVariableNode).varIndex]));
                        }
                        slots.Clear();
                    }
                }
