...
namespace UnityEditor.Graphing
{
    ...
    static class NodeUtils
    {
        ...
        public static void DepthFirstCollectNodesFromNode(List<AbstractMaterialNode> nodeList, AbstractMaterialNode node,
            ...
            // just in case there is a mismatch
            , bool searchLoopNode = false, int loopEndIndex = 0
            )
        {
            // no where to start
            if (node == null)
            ...
            if (searchLoopNode)
            {
                ...
                foreach (var slot in ids)
                {
                    foreach (var edge in node.owner.GetEdges(node.GetSlotReference(slot)))
                    {
                        ...
                        if (outputNode != null)
                        {
                            var slotDependentNodes = new List<AbstractMaterialNode>();
                            DepthFirstCollectNodesFromNode(slotDependentNodes, outputNode, keywordPermutation: keywordPermutation, ignoreActiveState: ignoreActiveState, searchLoopNode : searchLoopNode, loopEndIndex : loopEndIndex);
                            bool validStart = slotDependentNodes.Where(v=>((v is LoopStart) && ((v as LoopStart).loopIndex == loopEndIndex))).Count() > 0;
                            foreach(var slotDependentNode in slotDependentNodes)
                            {
                                if (!nodeList.Contains(slotDependentNode) && !dependentNodes.Contains(slotDependentNode))
                                {
                                    if (validStart)
                                        dependentNodes.Add(slotDependentNode);
                                    else
                                        nodeList.Add(slotDependentNode);
                                }
                            }
                        }
                    }
                ...
                if (dependentNodes.Count() > 0)
                    nodeList.AddRange(dependentNodes);
            }
            else
            {
                foreach (var slot in ids)
                {
                    foreach (var edge in node.owner.GetEdges(node.GetSlotReference(slot)))
                    {
                        var outputNode = edge.outputSlot.node;
                        if (outputNode != null)
                            DepthFirstCollectNodesFromNode(nodeList, outputNode, keywordPermutation: keywordPermutation, ignoreActiveState: ignoreActiveState, searchLoopNode : loopEnd, loopEndIndex: loopEnd ? (node as LoopEnd).loopIndex : 0);
                    }
                }
            }
            ...
            if (node.GetType() == typeof(GetLocalVariableNode) && node.owner.localVariables.Any() && node.owner.localVariables.Count > (node as GetLocalVariableNode).varIndex)
            {
                var localVar = node.owner.localVariables[(node as GetLocalVariableNode).varIndex];
                var outputNode = node.owner.GetLocalVariableRegisterNode(localVar);
                if (outputNode != null)
        ...
        // change as well, so update the "forest" or all connected trees of this node's leaves.
        ...
        public static void CollectNodesNodeFeedsInto(List<AbstractMaterialNode> nodeList, AbstractMaterialNode node, IncludeSelf includeSelf = IncludeSelf.Include)
        {
            ...
            if (node.GetType() == typeof(LocalVariableRegisterNode) && node.owner.getVarNodes.Any())
            {
                var getVariableNodes = node.owner.getVarNodes.Where(x=>x.variableName == (node as LocalVariableRegisterNode).variableName).ToList();
                foreach (var varNode in getVariableNodes)
                     CollectNodesNodeFeedsInto(nodeList, varNode);
            }
