...
namespace UnityEditor.ShaderGraph
{
    class ShaderGraphAnalytics
    {
        ...
        {
            internal const string k_CreationEventName = "shadergraphBuiltIn";
        ...
        {
            if (!EditorAnalytics.enabled || EditorAnalytics.RegisterEventWithLimit(ShaderGraphCreationData.k_CreationEventName, k_MaxEventsPerHour, k_MaxNumberOfElements, k_VendorKey) != AnalyticsResult.Ok)
            {
                return;
            }

            var data = new ShaderGraphCreationData()
            {
                action = action,
                materialType = materialType
            };

            EditorAnalytics.SendEventWithLimit(ShaderGraphCreationData.k_CreationEventName, data);
        }
