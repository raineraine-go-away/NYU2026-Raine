...
namespace UnityEditor.ShaderGraph
{
    ...
    class ShaderSubGraphImporter : ScriptedImporter
    {
        ...
        static void ProcessSubGraph(SubGraphAsset asset, GraphData graph, ShaderGraphImporter.AssetImportErrorLog importLog)
        {
            ...
            Dictionary<int, VGTiling> outputVGTilings = new Dictionary<int, VGTiling>();
            // If Sub Graph cannot take effect in the Frag stage, we do not need VGTiling.
            if ((effectiveShaderStage & ShaderStageCapability.Fragment) != 0)
            {
                foreach (var slot in outputSlots)
                {
                    // We should know which output RequiresVGTiling.
                    SlotRequireVGTiling slotRequireVGTiling = new SlotRequireVGTiling();
                    slotRequireVGTiling.slotId = slot.id;
                    slotRequireVGTiling.vgTiling = VGTiling.UV_INVALID;
                    var edges = graph.GetEdges(slot.slotReference);
                    if (edges.Any())
                    {
                        var fromSlot = edges.First().outputSlot;
                        var fromNode = fromSlot.node as IMayRequireOutputVGTiling;
                        slotRequireVGTiling.vgTiling = fromNode?.RequiresOutputVGTiling(fromSlot.slotId) ?? VGTiling.UV_INVALID;
                    }
                    asset.outputVGTilings.Add(slotRequireVGTiling);
                }
            }

            foreach (var outputVGTiling in asset.outputVGTilings)
            {
                outputVGTilings.Add(outputVGTiling.slotId, outputVGTiling.vgTiling);
            }
            ...
            registry.ProvideFunction(asset.functionName, asset.subGraphGraphPrecision, ConcretePrecision.Single, sb =>
            {
                ...
                foreach (MaterialSlot output in outputSlots)
                {
                    ...
                    // We should add output variable for VGTiling.
                    if (outputVGTilings.TryGetValue(output.id, out VGTiling vgTiling) && vgTiling != VGTiling.UV_INVALID)
                        arguments.Add($"out {asset.outputGraphPrecision.ToGenericString()}2 VGTiling_{output.id}");
                }
                ...
                sb.AppendLine("void {0}({1}{2})"
                    , asset.functionName
                    , arguments.Aggregate((current, next) => $"{current}, {next}")
                    , (arguments.Count == 0 ? "UNITY_GDRP_INSTANCE_UNIQUE_PARAMETER" : " UNITY_GDRP_INSTANCE_PARAMETER") + " , uint materialOffset = 0");
                ...
                using (sb.BlockScope())
                {
                    ...
                    foreach (var slot in outputSlots)
                    {
                        ...
                        // We should assign output variables for VGTiling.
                        if (outputVGTilings.TryGetValue(slot.id, out VGTiling vgTiling) && vgTiling != VGTiling.UV_INVALID)
                        {
                            var edges = graph.GetEdges(slot.slotReference);
                            if (edges.Any())
                            {
                                var fromSlot = edges.First().outputSlot;
                                var fromNode = fromSlot.node as IMayRequireOutputVGTiling;
                                sb.AppendLine($"VGTiling_{slot.id} = {fromNode.GetVGTilingVariableName(fromSlot.slotId)};");
                            }
                        }
                    }
