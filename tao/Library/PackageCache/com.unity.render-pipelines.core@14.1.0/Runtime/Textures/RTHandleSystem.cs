...
namespace UnityEngine.Rendering
{
    ...
    public partial class RTHandleSystem : IDisposable
    {
        ...
        /// <param name="resolveFromTextureMS">Resolve from MSAA texture instead of renderbuffer operations (for GLES and Msaa High Quality).</param>
        /// <returns></returns>
        public RTHandle Alloc(
            ...
            string name = "",
            bool resolveFromTextureMS = false
        )
        {
            return Alloc(width, height, wrapMode, wrapMode, wrapMode, slices, depthBufferBits, colorFormat, filterMode, dimension, enableRandomWrite, useMipMap,
                autoGenerateMips, isShadowMap, anisoLevel, mipMapBias, msaaSamples, bindTextureMS, useDynamicScale, memoryless, vrUsage, name, resolveFromTextureMS);
        }
        ...
        )
        {
            ...
            if (isShadowMap || depthBufferBits != DepthBits.None)
            {
                ...
                rt = new RenderTexture(width, height, (int)depthBufferBits, format, RenderTextureReadWrite.Linear)
                {
                    ...
                    resolveFromTextureMS = resolveFromTextureMS
                };
            }
            else
            {
