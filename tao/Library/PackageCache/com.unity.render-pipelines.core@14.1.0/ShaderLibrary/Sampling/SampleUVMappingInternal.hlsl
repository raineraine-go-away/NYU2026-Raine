...
real4 ADD_FUNC_SUFFIX(SampleUVMapping)(TEXTURE2D_PARAM(textureName, samplerName), UVMapping uvMapping EXTRA_TYPE_PARAMS)
{
    if (uvMapping.mappingType == UV_MAPPING_TRIPLANAR)
    {
        ...
        if (triplanarWeights.x > 0.0)
            val += triplanarWeights.x * SAMPLE_TEXTURE_FUNC_UVZY(textureName, samplerName);
        if (triplanarWeights.y > 0.0)
            val += triplanarWeights.y * SAMPLE_TEXTURE_FUNC_UVXZ(textureName, samplerName);
        if (triplanarWeights.z > 0.0)
            val += triplanarWeights.z * SAMPLE_TEXTURE_FUNC_UVXY(textureName, samplerName);

        return val;
    }
    else // UV_MAPPING_UVSET / UV_MAPPING_PLANAR
    {
        return SAMPLE_TEXTURE_FUNC_UV(textureName, samplerName);
    }
}
