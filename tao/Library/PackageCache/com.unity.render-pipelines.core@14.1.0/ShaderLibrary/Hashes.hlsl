...
void Hash_LegacyMod_3_3_float(float3 i, out float3 o)
{
    // Permutation and hashing used in webgl-nosie goo.gl/pX7HtC
    i = i % 289;
    // need full precision, otherwise half overflows when p > 1
    float x = float(34 * i.x + 1) * i.x % 289 + i.y + i.z;
    x = (34 * x + 1) * x % 289;
    x = frac(x / 41) * 2 - 1;

    float y = float(34 * i.y + 1) * i.y % 289 + i.x+ i.z;
    x = (34 * y + 1) * y % 289;
    x = frac(y / 41) * 2 - 1;

    float z = float(34 * i.z + 1) * i.z % 289 + i.x + i.y;
    x = (34 * z + 1) * z % 289;
    x = frac(z / 41) * 2 - 1;
...
void Hash_LegacyMod_3_3_half(half3 i, out half3 o)
{
    // Permutation and hashing used in webgl-nosie goo.gl/pX7HtC
    i = i % 289;
    // need full precision, otherwise half overflows when p > 1
    half x = half(34 * i.x + 1) * i.x % 289 + i.y + i.z;
    x = (34 * x + 1) * x % 289;
    x = frac(x / 41) * 2 - 1;

    half y = half(34 * i.y + 1) * i.y % 289 + i.x + i.z;
    x = (34 * y + 1) * y % 289;
    x = frac(y / 41) * 2 - 1;

    half z = half(34 * i.z + 1) * i.z % 289 + i.x + i.y;
    x = (34 * z + 1) * z % 289;
    x = frac(z / 41) * 2 - 1;
