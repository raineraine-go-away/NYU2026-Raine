...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed partial class UniversalRenderer : ScriptableRenderer
    {
        ...
        public override void Setup(ScriptableRenderContext context, ref RenderingData renderingData)
        {
            ...
            colorDescriptor.resolveFromTextureMS = false;
            if (IsGLESDevice() && (renderingData.cameraData.requiresOpaqueTexture || renderPassInputs.requiresColorTexture) && UniversalRenderPipeline.asset != null && UniversalRenderPipeline.asset.msaaHighQuality)
            {
                colorDescriptor.resolveFromTextureMS = true;
            }
        ...
        void CreateCameraRenderTarget(ScriptableRenderContext context, ref RenderTextureDescriptor descriptor, bool primedDepth, CommandBuffer cmd, ref CameraData cameraData)
        {
            using (new ProfilingScope(null, Profiling.createCameraRenderTarget))
            {
                ...
                if (m_CameraDepthAttachment == null || m_CameraDepthAttachment.nameID != BuiltinRenderTextureType.CameraTarget)
                {
                    ...
                    depthDescriptor.resolveFromTextureMS = false;
                    ...
                    // Enable bindMS for depth when MSAA optimization is enabled
                    if (IsGLESDevice() && hasMSAA && cameraData.requiresOpaqueTexture && UniversalRenderPipeline.asset != null && UniversalRenderPipeline.asset.msaaHighQuality)
                    {
                        depthDescriptor.bindMS = true;
                    }
