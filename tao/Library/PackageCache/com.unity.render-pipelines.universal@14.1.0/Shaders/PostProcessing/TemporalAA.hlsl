...
// gamma 2.0 is a good enough approximation
    ...
    half4 DoTemporalAA(Varyings input, int clampQuality, int motionQuality, int historyQuality, int centralFiltering)
    {
        ...
        half4 clampAccum = (clampQuality >= 3) ? ClipToAABBCenter(accum, boxMin, boxMax) : clamp(accum, boxMin, boxMax);
        ...
        half4 workingColor = ApplyHistoryColorLerp(clampAccum, colorCenter, frameInfluence);
