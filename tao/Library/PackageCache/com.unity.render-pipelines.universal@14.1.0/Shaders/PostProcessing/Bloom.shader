Shader "Hidden/Universal Render Pipeline/Bloom"
{
    HLSLINCLUDE
        ...
        half3 SampleBlitTexture(float2 uv, float2 offset)
        {
        ...
        half4 FragPrefilter(Varyings input) : SV_Target
        {
        ...
        #if _BLOOM_HQ
            float texelSize = _BlitTexture_TexelSize.x;
            half3 A = SampleBlitTexture(uv, float2(-1.0, -1.0));
            half3 B = SampleBlitTexture(uv, float2(0.0, -1.0));
            half3 C = SampleBlitTexture(uv, float2(1.0, -1.0));
            half3 D = SampleBlitTexture(uv, float2(-0.5, -0.5));
            half3 E = SampleBlitTexture(uv, float2(0.5, -0.5));
            half3 F = SampleBlitTexture(uv, float2(-1.0, 0.0));
            half3 G = SampleBlitTexture(uv, float2(0.0, 0.0));
            half3 H = SampleBlitTexture(uv, float2(1.0, 0.0));
            half3 I = SampleBlitTexture(uv, float2(-0.5, 0.5));
            half3 J = SampleBlitTexture(uv, float2(0.5, 0.5));
            half3 K = SampleBlitTexture(uv, float2(-1.0, 1.0));
            half3 L = SampleBlitTexture(uv, float2(0.0, 1.0));
            half3 M = SampleBlitTexture(uv, float2(1.0, 1.0));
            ...
            half3 color = o.xyz;
        #else
            half3 color = SampleBlitTexture(uv, float2(0.0, 0.0));
        #endif
