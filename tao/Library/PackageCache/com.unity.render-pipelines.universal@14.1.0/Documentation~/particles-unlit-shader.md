...
使用此 Shader 可用于不需要光照的粒子效果。由于不进行耗时的光照计算或查找，此 Shader 在低端硬件上具有较高的性能。Unlit Shader 采用通用渲染管线（URP）中最简单的[着色模型](shading-model.md)。

## 在编辑器中使用 Particles Unlit Shader

要选择并使用此 Shader，请执行以下步骤：

1. 在项目中创建或找到要应用此 Shader 的材质（Material）。选择该 __Material__，Material Inspector 窗口将打开。
2. 点击 __Shader__，然后选择 **Universal Render Pipeline** > **Particles** > **Unlit**。
...
![Particles Unlit Shader Inspector](Images/Inspectors/Shaders/ParticlesUnlit.png)
